...
The MERICA Core daemon can be configured to act as a trusted "border
router", implementing the merica wire protocol and relay, making
...
The ZeroMQ feature in MERICA Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building mericad:
...
For instance:
    ...
    $ mericad -zmqpubhashtx=tcp://127.0.0.1:246061 \
               -zmqpubhashtx=tcp://192.168.1.2:246061 \
               -zmqpubrawtx=ipc:///tmp/mericad.tx.raw \
...
These options can also be provided in merica.conf.
...
From the perspective of mericad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into mericad directly. Furthermore, no information is
...
using. MERICAd appends an up-counting sequence number to each
