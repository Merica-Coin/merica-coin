...
MERICA Core version 0.21.0 is now available from:

  <https://mericacore.org/bin/merica-core-0.21.0/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/merica/merica/issues>

To receive security and update notifications, please subscribe to:

  <https://mericacore.org/en/list/announcements/join/>
...
installer (on Windows) or just copy over `/Applications/MERICA-Qt` (on Mac)
or `mericad`/`merica-qt` (on Linux).

Upgrading directly from a version of MERICA Core that has reached its EOL is
possible, but it might take some time if the data directory needs to be migrated. Old
wallet versions of MERICA Core are generally supported.

Compatibility
==============

MERICA Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  MERICA
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use MERICA Core on
unsupported systems.

From MERICA Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, MERICA Core does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/merica/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by MERICA Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. MERICA Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/merica/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/merica/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/merica/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/merica/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/merica/bips/blob/master/bip-0342.mediawiki)),
...
- A new `merica-cli -netinfo` command provides a network peer connections
...
- A new `merica-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `merica-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `merica-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when MERICA Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `merica-wallet` tool which performs the salvage
...
MERICA Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`merica.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
MERICA Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `merica-cli createwallet` or `merica-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/merica/merica/issues).
...
Users who already have a MERICA Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of MERICA Core.
...
The change to using descriptors changes the default derivation paths used by MERICA Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in MERICA per 1,000
  vbytes (MERICA/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from MERICA/kB
  to sat/vB and the help documentation in MERICA/kB is updated to MERICA/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from MERICA/kvB to sat/vB and the
  ...
  difference between MERICA/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in MERICA/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time MERICA Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `merica.conf`
...
- #19284 Add seed.merica.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in merica-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add merica-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in merica-cli (laanwj)
...
### Build system
- #18504 Drop merica-tx and merica-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in MERICA_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for merica-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz MERICA Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitremerica)
...
- #18817 Document differences in mericad and merica-qt locale handling (practicalswift)
...
[Transifex](https://www.transifex.com/merica/merica/).
