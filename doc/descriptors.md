# Support for Output Descriptors in MERICA Core

Since MERICA Core v0.17, there is support for Output Descriptors. This is a
...
- Followed by the actual key, which is either:
  ...
  - [WIF](https://en.merica.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
  - `xpub` encoded extended public key or `xprv` encoded extended private key (as defined in [BIP 32](https://github.com/merica/bips/blob/master/bip-0032.mediawiki)).
...
- P2SH addresses (base58, of the form `3...` for mainnet or `2...` for testnet, defined in [BIP 13](https://github.com/merica/bips/blob/master/bip-0013.mediawiki)).
- Segwit addresses (bech32, of the form `bc1...` for mainnet or `tb1...` for testnet, defined in [BIP 173](https://github.com/merica/bips/blob/master/bip-0173.mediawiki)).
...
on MERICA's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
the entire path from the master down. [BIP174](https://github.com/merica/bips/blob/master/bip-0174.mediawiki) standardizes this by
...
existing MERICA Core wallets, a convenience function `combo` is
...
All RPCs in MERICA Core will include the checksum in their output. Only
