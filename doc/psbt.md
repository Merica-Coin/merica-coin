# PSBT Howto for MERICA Core

Since MERICA Core 0.17, an RPC interface exists for Partially Signed MERICA
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/merica/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for MERICA transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://mericatalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed MERICA transaction goes through the
following steps:
...
- An **Extractor** produces a valid MERICA transaction (in network format)
...
#### Multisig with multiple MERICA Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
MERICA Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `merica-cli` is used.
...
Later, when *V* MERICA has been received on *Amulti*, and Bob and Carol want to
