...
# Be aware that mericad and merica-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in mericad.
#
...
# C locale.
#
# In contrast, mericad does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where mericad is guaranteed to be running
# with the classic locale ("C") whereas the locale of merica-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in mericad but
# "1,230000" in merica-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/merica-tx.cpp.*stoul"
    "src/merica-tx.cpp.*trim_right"
