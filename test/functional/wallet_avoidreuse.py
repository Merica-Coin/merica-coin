...
class AvoidReuseTest(MERICATestFramework):
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        '''
        Test the same as test_sending_from_reused_address_fails, except send the 10 MERICA with
        the avoid_reuse flag set to false. This means the 10 MERICA send should succeed,
        ...
        # listunspent should show 1 single, unused 10 merica output
        assert_unspent(self.nodes[1], total_count=1, total_sum=10, reused_supported=True, reused_count=0)
        # getbalances should show no used, 10 merica trusted
        ...
        # listunspent should show 1 single, unused 5 merica output
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_supported=True, reused_count=0)
        # getbalances should show no used, 5 merica trusted
        ...
        # listunspent should show 2 total outputs (5, 10 merica), one unused (5), one reused (10)
        assert_unspent(self.nodes[1], total_count=2, total_sum=15, reused_count=1, reused_sum=10)
        # getbalances should show 10 used, 5 merica trusted
        assert_balances(self.nodes[1], mine={"used": 10, "trusted": 5})

        self.nodes[1].sendtoaddress(address=retaddr, amount=10, avoid_reuse=False)

        # listunspent should show 1 total outputs (5 merica), unused
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_count=0)
        # getbalances should show no used, 5 merica trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        # node 1 should now have about 5 merica left (for both cases)
    ...
    def test_sending_from_reused_address_fails(self, second_addr_type):
        ...
        [0] sends 10 MERICA to A.
        [1] spends 5 MERICA from A. (leaving roughly 5 MERICA useable)
        [0] sends 10 MERICA to A again.
        [1] tries to spend 10 MERICA (fails; dirty).
        [1] tries to spend 4 MERICA (succeeds; change address sufficient)
        '''
        ...
        if not self.options.descriptors:
            ...
            # listunspent should show 2 total outputs (1, 10 merica), one unused (1), one reused (10)
            assert_unspent(self.nodes[1], total_count=2, total_sum=11, reused_count=1, reused_sum=10)
            # getbalances should show 10 used, 1 merica trusted
            assert_balances(self.nodes[1], mine={"used": 10, "trusted": 1})

            # node 1 should now have about 1 merica left (no dirty) and 11 (including dirty)
    ...
    def test_full_destination_group_is_preferred(self):
        '''
        Test the case where [1] only has 11 outputs of 1 MERICA in the same reused
        address and tries to send a small payment of 0.5 MERICA. The wallet
        should use 10 outputs from the reused address as inputs and not a
        single 1 MERICA input, in order to join several outputs from the reused
        address.
        '''
        ...
        # Send 11 outputs of 1 MERICA to the same, reused address in the wallet
        for _ in range(11):
            self.nodes[0].sendtoaddress(new_addr, 1)
    ...
    def test_all_destination_groups_are_used(self):
        '''
        Test the case where [1] only has 22 outputs of 1 MERICA in the same reused
        address and tries to send a payment of 20.5 MERICA. The wallet
        ...
        # Send 22 outputs of 1 MERICA to the same, reused address in the wallet
        for _ in range(22):
            self.nodes[0].sendtoaddress(new_addr, 1)
