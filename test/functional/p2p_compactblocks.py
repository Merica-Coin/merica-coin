...
# TestP2PConn: A peer we use to send messages to mericad, and store responses.
...
class CompactBlocksTest(MERICATestFramework):
    ...
    # This test actually causes mericad to (reasonably!) disconnect us, so do this last.
    ...
    # Test that mericad requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for mericad to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # mericad will not send blocktxn responses for blocks whose height is
