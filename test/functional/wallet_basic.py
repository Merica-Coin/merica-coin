...
class WalletTest(MERICATestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 MERICA from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 100 merica in block rewards plus fees, but
        ...
        # Send 10 MERICA with subtract fee from amount
        ...
        # Sendmany 10 MERICA with subtract fee from amount
        ...
        fee_rate_sat_vb = 2
        fee_rate_merica_kvb = fee_rate_sat_vb * 1e3 / 1e8
        explicit_fee_rate_merica_kvb = Decimal(fee_rate_merica_kvb) / 1000
        ...
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_merica_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        self.sync_all(self.nodes[0:3])
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - amount, explicit_fee_rate_merica_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(balance, node_2_bal)
        node_0_bal += amount
        assert_equal(self.nodes[0].getbalance(), node_0_bal)
        ...
        for target, mode in product([-1, 0], ["merica/kb", "sat/b"]):
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid MERICA address
            assert_raises_rpc_error(-5, "Invalid MERICA address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid MERICA address
            assert_raises_rpc_error(-5, "Invalid MERICA address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid MERICA address or script", self.nodes[0].importaddress, "invalid")
            ...
            assert_fee_amount(fee, tx_size, Decimal(fee_rate_merica_kvb))
