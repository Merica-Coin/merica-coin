...
class ToolWalletTest(MERICATestFramework):
    ...
    def merica_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/merica-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-chain=%s' % self.chain] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_raises_tool_error(self, error, *args):
        p = self.merica_wallet_process(*args)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `merica-wallet help` raises an error. Use `merica-wallet -help`.
        ...
        if self.options.descriptors:
            error = "SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another mericad?"
    ...
    def test_salvage(self):
        # TODO: Check salvage actually salvages and doesn't break things. https://github.com/merica/merica/issues/7463
