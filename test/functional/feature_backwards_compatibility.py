...
class BackwardsCompatibilityTest(MERICATestFramework):
    ...
    def run_test(self):
        ...
        # RPC loadwallet failure causes mericad to exit, in addition to the RPC
        # call failure, so the following test won't work:
        # assert_raises_rpc_error(-4, "Wallet loading failed.", node_v17.loadwallet, 'w3_v18')

        # Instead, we stop node and try to launch it with the wallet:
        self.stop_node(4)
        node_v17.assert_start_raises_init_error(["-wallet=w3_v18"], "Error: Error loading w3_v18: Wallet requires newer version of MERICA Core")
        if self.options.descriptors:
            ...
            node_v17.assert_start_raises_init_error(["-wallet=w3"], "Error: Error loading w3: Wallet requires newer version of MERICA Core")
        self.start_node(4)
        ...
        # Copy the 0.16 wallet to the last MERICA Core version and open it:
        shutil.copyfile(
            os.path.join(node_v16_wallets_dir, "wallets/u1_v16"),
            os.path.join(node_master_wallets_dir, "u1_v16")
        )
        load_res = node_master.loadwallet("u1_v16")
        # Make sure this wallet opens without warnings. See https://github.com/merica/merica/pull/19054
        ...
        # Copy the 0.17 wallet to the last MERICA Core version and open it:
        ...
        # Copy the 0.19 wallet to the last MERICA Core version and open it:
        shutil.copytree(
