...
class ChainstateWriteCrashTest(MERICATestFramework):
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            try:
                ...
                # If mericad exits, then try again.  wait_for_node_exit()
                # should raise an exception if mericad doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, mericad isn't coming back up on restart.  Could be a
        # bug in mericad, or we've gotten unlucky with our dbcrash ratio --
