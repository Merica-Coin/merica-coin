...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The Travis linter also checks this, but [possibly not in all cases](https://github.com/merica/merica/pull/14884#discussion_r239585126).
...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the MERICATestFramework, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to control whether
  ...
  has 25 mature blocks (25x50=1250 MERICA) in its wallet.
...
with the mericad(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a mericad.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the MERICA Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the MERICATestFramework
...
Taken from the [python-mericarpc repository](https://github.com/jgarzik/python-mericarpc).
...
Test objects for interacting with a mericad node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-mericalib)
