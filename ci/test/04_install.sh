...
env | grep -E '^(MERICA_CONFIG|BASE_|QEMU_|CCACHE_|LC_ALL|BOOST_TEST_RANDOM|DEBIAN_FRONTEND|CONFIG_SHELL|(ASAN|LSAN|TSAN|UBSAN)_OPTIONS|PREVIOUS_RELEASES_DIR)' | tee /tmp/env
if [[ $MERICA_CONFIG = *--with-sanitizers=*address* ]]; then # If ran with (ASan + LSan), Docker needs access to ptrace (https://github.com/google/sanitizers/issues/764)
...
if [ ! -d ${DIR_QA_ASSETS} ]; then
  DOCKER_EXEC git clone --depth=1 https://github.com/merica-core/qa-assets ${DIR_QA_ASSETS}
fi
