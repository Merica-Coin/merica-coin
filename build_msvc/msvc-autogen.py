...
def main():
    parser = argparse.ArgumentParser(description='MERICA-core msbuild configuration initialiser.')
        ...
        with open(vcxproj_filename + '.in', 'r', encoding='utf-8') as vcxproj_in_file:
            with open(vcxproj_filename, 'w', encoding='utf-8') as vcxproj_file:
                vcxproj_file.write(vcxproj_in_file.read().replace(
                    '@SOURCE_FILES@\n', content))
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/merica_config.h'), os.path.join(SOURCE_DIR, 'config/merica-config.h'))
