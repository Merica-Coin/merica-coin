    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return MERICAUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, MERICAUnits::SeparatorStyle::NEVER);
            else
                return MERICAUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+MERICAUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
