...
#include <qt/mericaaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MERICA address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new MERICAAddressEntryValidator(parent));
    widget->setCheckValidator(new MERICAAddressCheckValidator(parent));
}

bool parseMERICAURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no merica: URI
    if(!uri.isValid() || uri.scheme() != QString("merica"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MERICAUnits::parse(MERICAUnits::MERICA, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMERICAURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMERICAURI(uriInstance, out);
}

QString formatMERICAURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("merica:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MERICAUnits::format(MERICAUnits::MERICA, info.amount, false, MERICAUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMERICAConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", MERICA_CONF_FILENAME));
    ...
    /* Open merica.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MERICA.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MERICA (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("MERICA (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for MERICA*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "merica.desktop";
    return GetAutostartDir() / strprintf("merica-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a merica.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=MERICA\n";
        else
            optionFile << strprintf("Name=MERICA (%s)\n", chain);
