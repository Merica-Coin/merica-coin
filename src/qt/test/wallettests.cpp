...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<MERICAAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_merica-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_merica-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_merica-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = MERICAUnits::formatWithUnit(unit, balance, false, MERICAUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    MERICAAmountField* amountInput = receiveCoinsDialog.findChild<MERICAAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_merica-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
