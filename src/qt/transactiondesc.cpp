...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord *rec, int unit)
{
    ...
    if (wtx.is_coinbase && nCredit == 0)
    {
        ...
        if (status.is_in_main_chain)
            strHTML += MERICAUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", status.blocks_to_maturity) + ")";
        else
    ...
    else if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + MERICAUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
        ...
        if (fAllFromMe)
        {
            ...
            for (const CTxOut& txout : wtx.tx->vout)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + MERICAUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if(toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + MERICAUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            ...
            if (fAllToMe)
            {
                ...
                strHTML += "<b>" + tr("Total debit") + ":</b> " + MERICAUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + MERICAUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = nDebit - wtx.tx->GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + MERICAUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            //
            ...
            for (const CTxIn& txin : wtx.tx->vin) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Debit") + ":</b> " + MERICAUnits::formatHtmlWithUnit(unit, -wallet.getDebit(txin, ISMINE_ALL)) + "<br>";
                }
            }
            mine = wtx.txout_is_mine.begin();
            for (const CTxOut& txout : wtx.tx->vout) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Credit") + ":</b> " + MERICAUnits::formatHtmlWithUnit(unit, wallet.getCredit(txout, ISMINE_ALL)) + "<br>";
                }
            }
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + MERICAUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal merica:URI (merica:123...?message=example)
            ...
            {
                {
                    ...
                    strHTML = strHTML + " " + tr("Amount") + "=" + MERICAUnits::formatHtmlWithUnit(unit, vout.nValue);
