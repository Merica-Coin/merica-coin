This directory contains the MERICAQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the MERICA UI.
...
ClientModel is used by the main application `mericagui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a merica:... link from a browser).
...
* UI elements like MERICAAmountField, which inherit from QWidget.
* `mericastrings.cpp`: automatically generated
* `mericaunits.(h/cpp)`: MERICA / mMERICA / etc handling
* `callback.h`
...
* don't change `local/merica_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
4. Enter "merica-qt" as project name, enter src/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "merica-qt" under "Run", which is where you can also add command line arguments)
