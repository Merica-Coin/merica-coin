...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = MERICAUnits::format(m_wallet_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
std::string PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(MERICAUnits::formatWithUnit(MERICAUnits::MERICA, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(MERICAUnits::formatWithUnit(MERICAUnits::MERICA, *analysis.fee));
        ...
        tx_description.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
            .arg(MERICAUnits::formatHtmlWithUnit(m_client_model->getOptionsModel()->getDisplayUnit(), totalAmount)));
