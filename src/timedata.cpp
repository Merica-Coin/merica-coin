...
#define MERICA_TIMEDATA_MAX_SAMPLES 200
...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample)
{
    ...
    if (setKnown.size() == MERICA_TIMEDATA_MAX_SAMPLES)
        return;
    if (!setKnown.insert(ip).second)
        return;

    // Add data
    static CMedianFilter<int64_t> vTimeOffsets(MERICA_TIMEDATA_MAX_SAMPLES, 0);
