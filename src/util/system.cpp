...
const char * const MERICA_CONF_FILENAME = "merica.conf";
const char * const MERICA_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/merica/merica/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //merica-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", MERICA_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\MERICA
    // macOS: ~/Library/Application Support/MERICA
    // Unix-like: ~/.merica
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MERICA";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/MERICA";
#else
    // Unix-like
    return pathRet / ".merica";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MERICA_CONF_FILENAME);
