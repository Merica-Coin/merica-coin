 ...
 */
static const char* MERICA_PID_FILENAME = "mericad.pid";

static fs::path GetPidFile(const ArgsManager& args)
{
    return AbsPathForConfigVal(fs::path(args.GetArg("-pid", MERICA_PID_FILENAME)));
}
...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location. (default: %s)", MERICA_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", MERICA_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-settings=<file>", strprintf("Specify path to dynamic settings data file. Can be disabled with -nosettings. File is written at runtime and not meant to be edited by users (use %s instead for custom settings). Relative paths will be prefixed by datadir location. (default: %s)", MERICA_CONF_FILENAME, MERICA_SETTINGS_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#if HAVE_SYSTEM
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ttracx/merica-coin>";
...
/** Sanity checks
 *  Ensure that MERICA is running in a usable environment with all
...
{
    // Make sure only a single MERICA process is using the data directory.
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    fs::path config_file_path = GetConfigFile(args.GetArg("-conf", MERICA_CONF_FILENAME));
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if merica is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if merica is started while in a temporary directory.\n",
