...
BOOST_AUTO_TEST_CASE(caddrdb_read)
{
    ...
    BOOST_CHECK(Lookup("250.7.1.1", addr1, 46062, false));
    ...
    CService source;
    BOOST_CHECK(Lookup("252.5.1.1", source, 46062, false));
...
BOOST_AUTO_TEST_CASE(cnetaddr_unserialize_v2)
{
    ...
    s << MakeSpan(ParseHex(
        ...
        "10"                                  // address length
        "fd6b88c08724ca978112ca1bbdcafac2")); // address: 0xfd + sha256("merica")[0:5] +
                                              // sha256(name)[0:10]
    s >> addr;
