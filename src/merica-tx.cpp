...
static void SetupMERICATxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " merica-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  merica-tx [options] <hex-tx> [commands]  Update hex-encoded merica transaction\n" +
            "or:     merica-tx [options] -create [commands]   Create hex-encoded merica transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded merica transaction
