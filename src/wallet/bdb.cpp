...
//! so merica should never create different databases with the same fileid, but
...
bool BerkeleyEnvironment::Open(bilingual_str& err)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of merica may be using it.\n", strPath);
...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    if (fs::exists(file_path))
    {
        ...
        if (result != 0) {
            errorStr = strprintf(_("%s corrupt. Try using the wallet tool merica-wallet to salvage or restoring a backup."), file_path);
            return false;
        }
    }
