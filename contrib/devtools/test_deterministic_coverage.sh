...
TEST_MERICA_BINARY="src/test/test_merica"
...
if [[ ! -e ${TEST_MERICA_BINARY} ]]; then
    echo "Error: Executable ${TEST_MERICA_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_MERICA_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
