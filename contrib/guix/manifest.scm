...
(define* (make-merica-cross-toolchain target
                                  #:key
...
desirable for building MERICA Core release binaries."
  ...
  (let ((target (getenv "HOST")))
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "riscv64-linux-")
           (list (make-merica-cross-toolchain "riscv64-linux-gnu"
                                               #:base-gcc-for-libc gcc-7)))
          ((string-contains target "-linux-")
           (list (make-merica-cross-toolchain target)))
          (else '())))))
