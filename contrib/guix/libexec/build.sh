...
# Setup the directory where our MERICA Core build for HOST will occur
...
make -C depends --jobs="$MAX_JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform merica-linux-g++'
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} MERICA_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our MERICA Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built MERICA Core to $INSTALLPATH
